...
mongoose
  .connect("mongodb://localhost:27017/M&B", {
...
app.get("/auth", (req, res) => {
  try {
    ...
    return res
      .status(401)
      .json({ authenticated: false, message: "Token inválido ou expirado." });
  }
});
...
app.post("/home/registar", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Utilizador registrado com sucesso.", user });
  } catch (err) {
    console.error("Erro ao registrar utilizador:", err);
    res
      .status(500)
      .json({ error: "Erro ao registrar utilizador: " + err.message });
  }
});
...
app.post("/addPlaylist", imagens.single("imagem"), async (req, res) => {
  try {
    ...
    if (!nome || !utilizador || !imagem) {
      return res
        .status(400)
        .json({ error: "Nome, utilizador e imagem são obrigatórios." });
    }
    ...
    if (musicas) {
      if (typeof musicas === "string") {
        try {
          ...
          return res
            .status(400)
            .json({ error: 'O campo "musicas" deve conter um JSON válido.' });
        }
...
app.get("/playlists", async (req, res) => {
  ...
  if (!authHeader) {
    return res
      .status(401)
      .json({ error: "Token ausente no cabeçalho de autorização." });
  }
...
app.post("/playlist/:playlistId/add-musica/:musicaId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Música adicionada com sucesso", playlist });
  } catch (error) {
